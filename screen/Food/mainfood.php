<?php
// Kết nối cơ sở dữ liệu
require('../../db/conn.php');

// Lấy dữ liệu từ bảng danh_muc_do_an
$sql = "SELECT * FROM danh_muc_do_an";
$result = mysqli_query($conn, $sql);

$menuItems = [];
if ($result && mysqli_num_rows($result) > 0) {
    while ($row = mysqli_fetch_assoc($result)) {
        $menuItems[] = $row;
    }
}
?>
<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="http://localhost/a/style/header.css">
    <link rel="stylesheet" href="http://localhost/a/style/index.css">
    <link rel="stylesheet" href="http://localhost/a/style/global.css">
    <link rel="stylesheet" href="http://localhost/a/style/mainscreen.css">
    <link rel="stylesheet" href="http://localhost/a/style/footer.css">
    <link rel="stylesheet" href="http://localhost/a/style/banner.css">
    <link rel="stylesheet" href="http://localhost/a/style/foodmain.css">
    <title>Thực đơn chính</title>
</head>

<body>
<?php
    require('../header.php');
    ?>
    <!-- <header class="header bg-black relative align-center flex flex-col p-0">
        <div class="head-navigation flex align-center">
            <img class="logo" src="data:image/webp;base64," alt="logo">
            <ul class="navigation-menu flex align-center">
                <li><a href="http://localhost/a/screen/index.html" class="text-white cursor-pointer decoration-none menu-text font-light home-navigation">Trang chủ</a></li>
                <li class="relative"><a href="" class="text-white cursor-pointer decoration-none menu-text font-light info-navigation ">Tìm hiểu</a></li>
                <li class="relative"><a href="http://localhost/a/screen/Food/mainfood.php" class="text-white cursor-pointer decoration-none menu-text font-light food-navigation text-secondary">Thực đơn</a></li>
                <li><a href="http://localhost/a/screen/discount.html" class="text-white cursor-pointer decoration-none menu-text font-light discount-navigation ">Khuyến mãi</a></li>
                <li><a href="" class="text-white cursor-pointer decoration-none menu-text font-light news-navigation">Tin tức</a></li>
                <li><a href="" class="text-white cursor-pointer decoration-none menu-text font-light contact-navigation">Liên hệ</a></li>
            </ul>
            <div class="ml-10 flex gap-10">
                <div class="navbtn-store othernav-btn bg-primary flex align-center cursor-pointer">
                <img class="" src="data:image/webp;base64," alt="">
                <div class="othernavbtn-content flex flex-col"><p class="text-white">Hệ thống</p><span class="text-secondary">Cửa hàng</span></div>
                </div>
                <div class="navbtn-delivery othernav-btn bg-primary flex align-center cursor-pointer">
                <img class="" src="data:image/webp;base64," alt="">
                <div class="othernavbtn-content flex flex-col"><p class="text-white">Giao hàng</p><span class="text-secondary">Mc Delivery</span></div>
                </div>
            </div>
        </div>
    </header> -->

    <div class="Custom-Slider relative">
        <div class="submenu no-display  justify-center info absolute w-full align-center cursor-pointer">
            <div class="flex submenu-content justify-between  gap-10 ">
                <a href="" class="decoration-none text-center" ><img src="http://localhost/a/assets/img/submenuImg/submenuItem.jpg" alt=""><p class="text-white">Lịch sử</p></a>
                <a href="" class="decoration-none text-center" ><img src="http://localhost/a/assets/img/submenuImg/submenuItem1.jpg" alt=""><p class="text-white">Giới thiệu</p></a>
                <a href="" class="decoration-none text-center" ><img src="http://localhost/a/assets/img/submenuImg/submenuItem2.jpg" alt=""><p class="text-white">Xuất xứ</p></a>
                <a href="" class="decoration-none text-center" ><img src="http://localhost/a/assets/img/submenuImg/submenuItem3.jpg" alt=""><p class="text-white">Nhà cung cấp</p></a>
                <a href="" class="decoration-none text-center" ><img src="http://localhost/a/assets/img/submenuImg/submenuItem4.jpg" alt=""><p class="text-white">Dịch vụ</p></a>
                <a href="" class="decoration-none text-center" ><img src="http://localhost/a/assets/img/submenuImg/submenuItem5.jpg" alt=""><p class="text-white">An toàn</p></a>
            </div>
        </div>

        <div class="slider">
            <div class="slides">
                <div class="slide active">
                    <img src="http://localhost/a/assets/img/slider/sliderImg.png" alt="Slide 1">
                </div>
                <div class="slide">
                    <img src="http://localhost/a/assets/img/slider/sliderImg1.png" alt="Slide 2">
                </div>
                <div class="slide">
                    <img src="http://localhost/a/assets/img/slider/sliderImg2.png" alt="Slide 3">
                </div>
            </div>
            <button class="prev btnSlides" onclick="changeSlide(-1)">&#10094;</button>
            <button class="next btnSlides" onclick="changeSlide(1)">&#10095;</button>
            <script src="http://localhost/a/js/banner.js"></script>
        </div>
    </div>


    <div class="mainfood">
    <div class="mainscreen">
        <div class="menu-filtered-section justify-center flex j bg-dark-gray cursor-pointer menudefault">
            <div class="container flex">
                <?php 
                // Hàm chuyển đổi chuỗi có dấu thành không dấu và định dạng slug
                function toSlug($string) {
                    $string = mb_strtolower($string, 'UTF-8'); // Chuyển thành chữ thường
                    $string = preg_replace('/[áàảạãăắằẳẵặâấầẩẫậ]/u', 'a', $string);
                    $string = preg_replace('/[éèẻẽẹêếềểễệ]/u', 'e', $string);
                    $string = preg_replace('/[íìỉĩị]/u', 'i', $string);
                    $string = preg_replace('/[óòỏõọôốồổỗộơớờởỡợ]/u', 'o', $string);
                    $string = preg_replace('/[úùủũụưứừửữự]/u', 'u', $string);
                    $string = preg_replace('/[ýỳỷỹỵ]/u', 'y', $string);
                    $string = preg_replace('/[đ]/u', 'd', $string);
                    $string = preg_replace('/[^a-z0-9]+/u', '', $string); // Xóa ký tự đặc biệt
                    return $string;
                }
                ?>

                <?php foreach ($menuItems as $index => $menuItem): ?>
                    <?php 
                        // Chuyển đổi tên danh mục thành slug không dấu
                        $pageSlug = toSlug($menuItem['ten_danh_muc']) . ".php";
                    ?>
                    <div class="relative grow text-center main-food" onclick="window.location.href='<?= $pageSlug ?>'">
                        <h2 class="font-light text-white"><?= $menuItem['ten_danh_muc']; ?></h2>
                        <div class="triangle no-display absolute center main-tria"></div>
                    </div>
                <?php endforeach; ?>
            </div>
        </div>
    </div>
</div>


            <div class="menu-Items-section flex justify-center mt-55">
                <div class="container grid-1vs2 gap-20">
                    <div class="food-content flex">
                        <div class="food-content-bruh">
                            <h2 class="font-light text-primary">Phần ăn EVM</h2>
                            <p>Phần ăn gồm món chính kèm khoai và nước cho 1 người, tiết kiệm 29%</p>
                            <button onclick=" window.location.href='menudetail.php'" class="font-light">Khám phá</button>
                        </div>
                        <img src="https://mcdonalds.vn/uploads/2018/food-categories/xEVM_regular,281,29.png.pagespeed.ic.gUbdnHJ3qj.webp" alt="">
                    </div>
                    <div class="food-content flex">
                        <div class="food-content-bruh">
                            <h2 class="font-light text-primary">Combo từ 39K</h2>
                            <p class="food-content-bruh">Các combo cơm gà rán, burger kèm khoai và nước chỉ từ 39K. No nê mỗi ngày tại McDonald's.</p>
                            <button onclick=" window.location.href='menucombo39k.php'" class="font-light">Khám phá</button>
                        </div>
                        <img src="https://mcdonalds.vn/uploads/2018/food-categories/xEVM_regular,281,29.png.pagespeed.ic.gUbdnHJ3qj.webp" alt="">
                    </div>
                  
                </div>
            </div>
        </div>
       

        <div class="flex flex-col align-center mt-55">
            <div class="footer">
                <!-- Nội dung của footer -->
            </div>
            <div class="bg-black w-full footer-last mt-35 text-white font-light">
                ©2017-2018 McDonald's. All Rights Reserved.
            </div>
        </div>
    </div>

    <script src="http://localhost/a/js/banner.js"></script>
    <script src="http://localhost/a/js/mainfood.js"></script>
    <script src="http://localhost/a/js/header.js"></script>
    <script src="http://localhost/a/js/navigate.js"></script>
</body>

</html>
